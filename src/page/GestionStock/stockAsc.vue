    ...
    <div class="row column_title">
      <div class="col-md-12">
        <div class="page_title">
          <h2 style="font-weight: bolder;font-family:Georgia, 'Times New Roman', Times, serif;">STOCK ASC</h2>
        </div>
      </div>
    </div>
...
export default {
  ...
  computed: {
    ...
    afficherEquipementEnfonctionType() {
      ...
      this.getterEquipement.filter((item) => {
        if (
          item.type_equipement_id === this.objetrenoule.type_equipement_id &&
          item.quantite != 0
        ) {
          let data = {
            id: item.id,
  ...
  return (parseInt($id)-parseInt($id1))
    ...
    async AfficheModalAjoute(id) {
      this.ObjetAjoute = this.getterAscConnecter.find(
